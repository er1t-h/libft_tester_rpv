...
test!(
    ft_lstadd_front(elements: DisplayableStringSlice<&str>, pre_add: usize) {
        ...
        let mut list: TListHandle = begin.iter().rev().map(|x| x.as_ptr().cast_mut()).collect();

        debug_assert_eq!(list.len(), begin.len(), "DPS: FromIter didn't work");

        for item in end.iter().map(|x| x.as_ptr().cast_mut().cast()) {
            list.add_front(TListNode::new_panicking(item));
        }

        assert_eq!(list.len(), elements.len(), "the size of the lists mismatch");
        assert_eq!(list.last().map(|x| unsafe { CStr::from_ptr((*x).data.cast()) }), elements.first().map(|x| x.as_c_str()), "last returned the wrong element");

        for (i, (libft, libc)) in list.iter().map(|x| unsafe { CStr::from_ptr(x.cast()) }).zip(elements.iter().rev()).enumerate() {
...
crate::fork_test! {
    ...
    fn random() {
        ...
        eprintln!("ft_lstadd_front(`elements`, `element_present_at_start`)");
        eprintln!("The first `element_present_at_start` are already in the list (in the same order lst_addfront would have added them). The remaining elements will be added by lstadd_front");
