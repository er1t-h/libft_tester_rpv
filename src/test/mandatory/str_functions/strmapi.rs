	...
	($name: ident, $str: expr, $func: ident) => {
		crate::fork_test!{
			...
			fn $name() {
				...
				assert_eq!(expected.collect::<Vec<u8>>().as_slice().cmp(bytes), Ordering::Equal);
...
test!(longer_rotone, "Un super test de qualite n'est-ce pas", rotone);
test!(basic_rotx, "Bonjour", rotx);
test!(longer_rotx, "Un super test de qualite n'est-ce pas", rotx);
test!(basic_to_num, "Bonjour", to_num);
test!(longer_to_num, "Un super test de qualite n'est-ce pas", to_num);
