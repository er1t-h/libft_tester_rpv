	...
	($name: ident, $s1: expr, $s2: expr) => {
		crate::fork_test!{
			...
			fn $name() {
				...
				let content = unsafe { from_raw_parts(user_ret as *mut u8, s1.as_bytes().len() + s2.as_bytes().len()) };
				assert_eq!([s1.as_bytes(), s2.as_bytes()].concat(), content);
...
crate::fork_test!{
	...
	fn first_null() {
		...
		let content = unsafe { from_raw_parts(user_ret as *mut u8, s2.as_bytes().len()) };
		if s2.as_bytes() == content {
			unsafe {libc::free(user_ret as *mut libc::c_void)};
...
crate::fork_test!{
	...
	fn second_null() {
		...
		let content = unsafe { from_raw_parts(user_ret as *mut u8, s1.as_bytes().len()) };
		if s1.as_bytes() == content {
			unsafe {libc::free(user_ret as *mut libc::c_void)};
